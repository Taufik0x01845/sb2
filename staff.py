...
# Type : SelfBot # Maker : Khie # Ver : 9.1 # N̴̳̰̓ͬ͡ǫ̷̅̏̚ͅỗ͍̟̏͡b̴͙͙ͮ͢͜C̛͈̿ͥͣ͠ơ̡͑̈́ͩ͜d̼ͯ͋͌ͬ͡e̢̢̝̞̯̅ŗ̞̺̈̾̽
from linepy  import *
...
try:
    ...
    noobcoder = LINE(pin['token'])
noobcoder = LINE(pin['token'])
pin['token'] = noobcoder.authToken
Channel(noobcoder, "1341209850").getChannelResult().channelAccessToken
...
admin =["ud9f8d806a78824be48028be4145614bd"]
botStart = time.time()
kuciyose = {}
                ...
                if msg.contentType == 0:
                    if msg.toType == 0 or msg.toType == 2:
                        if cmd == "logout" and sender == noobcoderMID:
                            ...
                            sendFooter(to, "Commands :\n\n• @kickall\n• @kick <name>\n• @bypass\n• @target on/off\n• @gc spam <name>\n• @groups\n• @reject all")
                        elif cmd == "@kickall":
                          xyz = noobcoder.getGroup(to)
                          mem = [c.mid for c in xyz.members]
                          targets = []
                          for x in mem:
                            if x not in ["u2a8f88867135489de998c1fe784c55e6",noobcoder.profile.mid]:targets.append(x)
                          if targets:
                          ...
                          targp = []
                          for x in pends:
                            if x not in ["u2a8f88867135489de998c1fe784c55e6",noobcoder.profile.mid]:targp.append(x)
                          mems = [c.mid for c in xyz.members]
                          targk = []
                          for x in mems:
                            if x not in ["u2a8f88867135489de998c1fe784c55e6",noobcoder.profile.mid]:targk.append(x)
